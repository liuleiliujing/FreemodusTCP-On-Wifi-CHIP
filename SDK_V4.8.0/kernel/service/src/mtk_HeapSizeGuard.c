...
#endif /* __GNUC__ */
{
    ...
    if (xwanted_size != GET_ACTUAL_BLOCK_SIZE(xwanted_size)) {
        ...
        LOGI("mtk_HeapSizeGuard: too large of a request. Do not enable this feature\n");
        return (NULL);
    }
    ...
    if (xBlockSize >= SPECIFIC_SIZE) {
        if (xTaskGetSchedulerState() == taskSCHEDULER_NOT_STARTED) {
            ...
            LOGI("H_Specific_A:%d,0x%x,%d,%d,InitStage,0x%x\r\n",
                 (unsigned int)xTaskGetTickCount(),
                 (unsigned int)pvReturn,
                 xwanted_size,
                 xBlockSize,
            ...
            /*H_Specific_A:tick,addr,userSize,blockSize,taskName,lr*/
            LOGI("H_Specific_A:%d,0x%x,%d,%d,%s,0x%x\r\n",
                 (unsigned int)xTaskGetTickCount(),
                 (unsigned int)pvReturn,
                 xwanted_size,
                 xBlockSize,
