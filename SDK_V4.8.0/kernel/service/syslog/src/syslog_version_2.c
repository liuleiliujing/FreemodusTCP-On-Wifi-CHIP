...
static int32_t pc_package_align_and_assembly(uint8_t sub_type, uint32_t payload_len, bool end_flag, uint8_t *respond_buffer)
{
    uint8_t *p_buffer;
    uint8_t i, align_len;
    uint32_t *p_header;
...
static volatile uint8_t *find_start_of_cpu_log_filters(uint32_t cpu_id)
{
    uint32_t offset, i;
...
#define MAX_DROP_PROMPT_LOG_SIZE DROP_PROMPT_LOG_FIX_SIZE + 10 + 2 + 3 /* drop count, \r\n, align bytes */
...
static uint32_t buffer_calculate_free_space(uint32_t hw_rptr, uint32_t hw_wptr, uint32_t urgent_log_size)
{
    uint32_t free_space;
...
static uint8_t *buffer_copy_data(uint8_t *src_buf, uint8_t *dst_buf, uint32_t log_size)
{
    uint32_t i;
    uint8_t *p_buf;
...
uint8_t *log_buffer_pre_alloc(uint32_t log_size, bool is_urgent_log, uint32_t *timestamp)
{
    uint32_t free_space;
    uint32_t p_log_fill, p_log_hw_rptr, p_log_hw_wptr;

    port_syslog_global_enter_critical();
    ...
    if (is_urgent_log == false) {
        free_space = buffer_calculate_free_space(p_log_hw_rptr, p_log_hw_wptr, MAX_URGENT_LOG_LENGTH + MAX_DROP_PROMPT_LOG_SIZE);
    } else {
        free_space = buffer_calculate_free_space(p_log_hw_rptr, p_log_hw_wptr, 0);
    }

    if (free_space < log_size) {
        g_syslog_share_variable->drop_count++;
        port_syslog_global_exit_critical();
        return NULL;
    }
    ...
    if (timestamp) {
        *timestamp = port_syslog_get_current_timestamp();
    }

    port_syslog_global_exit_critical();

    return (uint8_t *)share_log_buffer_address(p_log_fill);
}

static void drop_prompt_log_insert(void)
{
    uint8_t str_array[16];
    uint8_t *p_curr_wptr;
    uint32_t p_log_hw_rptr, p_log_hw_wptr, tlv_header[3];
    uint32_t i, bits, free_space, drop_log_size, total_size, align_byte;

    /* Get the RPTR and WPTR */
    p_log_hw_rptr = port_syslog_device_get_hw_rptr(g_syslog_share_variable->port_type, g_syslog_share_variable->port_index);
    p_log_hw_wptr = port_syslog_device_get_hw_wptr(g_syslog_share_variable->port_type, g_syslog_share_variable->port_index);

    /* Calculate the size of drop log */
    bits = uint_to_str(g_syslog_share_variable->drop_count, str_array);
    drop_log_size = DROP_PROMPT_LOG_FIX_SIZE + bits;
    total_size = drop_log_size + TLV_LOG_HEADER_SIZE; /* add TLV header */
    ALIGNMENT_TO_4_BYTE_BOUNDARY(total_size, align_byte);
    total_size += align_byte;

    /* Check whether drop log can be inserted */
    free_space = buffer_calculate_free_space(p_log_hw_rptr, p_log_hw_wptr, MAX_URGENT_LOG_LENGTH);
    if (free_space >= total_size) {
        p_curr_wptr = (uint8_t *)share_log_buffer_address(g_syslog_share_variable->sw_wptr);
        TLV_NORMAL_LOG_FORMAT_HEADER(tlv_header, TEXT_LOG_TYPE_PRINTF, drop_log_size, align_byte, total_size);
        TLV_INSERT_TIMESTAMP(tlv_header, port_syslog_get_current_timestamp());
        p_curr_wptr = buffer_copy_data((uint8_t *)tlv_header, p_curr_wptr, TLV_LOG_HEADER_SIZE);
        p_curr_wptr = buffer_copy_data((uint8_t *)DROP_PROMPT_LOG, p_curr_wptr, DROP_PROMPT_LOG_FIX_SIZE);
        p_curr_wptr = buffer_copy_data(str_array, p_curr_wptr, bits);
        for (i = 0; i < align_byte; i++) {
            str_array[i] = 0;
        }
        p_curr_wptr = buffer_copy_data(str_array, p_curr_wptr, align_byte);
        if (total_size >= (PORT_SYSLOG_SHARE_MEMORY_SIZE - g_syslog_share_variable->sw_wptr)) {
            g_syslog_share_variable->sw_wptr = total_size - (PORT_SYSLOG_SHARE_MEMORY_SIZE - g_syslog_share_variable->sw_wptr);
        } else {
            g_syslog_share_variable->sw_wptr += total_size;
        }
        g_syslog_share_variable->drop_count = 0;
    }
}

void log_buffer_wptr_update(void)
{
    ...
    if (g_syslog_share_variable->nest_count == 0) {
        /* check whether need to insert the log for drop prompt */
        if (g_syslog_share_variable->drop_count) {
            drop_prompt_log_insert();
        }
...
static int32_t pc_get_cpu_information(uint8_t sub_type, bool need_reset, uint32_t payload_len, uint8_t *respond_buffer)
{
    ...
    strcpy((char *)&p_respond_buffer[1 + CMD_CPU_NAME_MAX_LENGTRH], (char *)(PORT_SYSLOG_SDK_VERSION_BEGIN + cpu_id * PORT_SYSLOG_SDK_VERSION_LENGTH));
    strcpy((char *)&p_respond_buffer[1 + CMD_CPU_NAME_MAX_LENGTRH + CMD_SDK_VERSION_MAX_LENGTRH], (char *)(PORT_SYSLOG_BUILD_TIME_BEGIN + cpu_id * PORT_SYSLOG_BUILD_TIME_LENGTH));
...
static void pc_initial_sync(void)
{
    ...
    p_buf = log_buffer_pre_alloc(total_size, false, NULL);
    if (p_buf == NULL) {
        assert(0);
    }
...
static bool check_log_control(const void *is_module_control, log_control_block_t *context, print_level_t level)
{
...
static void string_log_handler(void *handle,
                               const char *func,
                               int line,
                               print_level_t level,
                               const char *message,
...
static int32_t binary_log_handler(void *handle,
                                  print_level_t level,
                                  log_type_t type,
                                  const char *message,
                                  const void **p_data,
...
void vprint_module_msgid_log(void *handle,
                             print_level_t level,
                             const char *message,
                             uint32_t arg_cnt,
                             va_list list)
                            ...
                            uint32_t arg_cnt, ...)
{
    ...
    vprint_module_msgid_log(handle, level, message, arg_cnt, list);
    va_end(list);
}
