...
extern uint16_t  modbus_connect_flag;
...
xMBTCPPortInit( USHORT usTCPPort )
{
    ...
    if( usTCPPort == 0 )
    {
        usPort = MB_TCP_DEFAULT_PORT;  //502
    }
    else
    {
        usPort = ( USHORT ) usTCPPort;  //默认端口502
    }
...
#endif
		 //printf("\r\n --%s,LINE=%d,tcpbufpos=%d\r\n",__func__,__LINE__,usTCPBufPos);
