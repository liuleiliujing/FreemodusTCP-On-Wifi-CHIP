...
uint8_t encrypted_cbc_efuse_key0[] = {
    0x7d ,0x48 ,0xbf ,0x7f ,0xcd ,0xea ,0x07 ,0x6d ,0x5f ,0x83 ,0xb3 ,0xef ,0xec ,0xf2 ,0x26 ,0xb6 ,
    0xb8 ,0x69 ,0x83 ,0x4a ,0x66 ,0x50 ,0xe8 ,0xb3 ,0x7a ,0xcd ,0x3b ,0xcb ,0x3b ,0x75 ,0xf2 ,0x9f
};

uint8_t encrypted_ecb_efuse_key0[] = {
    0xf0 ,0x90 ,0xb9 ,0x87 ,0x5d ,0xfd ,0x3f ,0xbe ,0xb9 ,0x80 ,0xaf ,0x74 ,0xb7 ,0x8b ,0x9d ,0xe3 ,
    0x75 ,0x20 ,0xf8 ,0x55 ,0xfc ,0x1e ,0x83 ,0xa9 ,0x32 ,0x7b ,0xa4 ,0x82 ,0x7f ,0xa8 ,0xa1 ,0x2f
};

uint8_t encrypted_cbc_efuse_key1[] = {
    0x6f ,0xa4 ,0x70 ,0x7f ,0xde ,0xcb ,0xf8 ,0x44 ,0x04 ,0x33 ,0x83 ,0x1b ,0x44 ,0x57 ,0x03 ,0x3c ,
    0x11 ,0xf1 ,0xe1 ,0x95 ,0x3b ,0x02 ,0xa8 ,0x73 ,0xcb ,0xf0 ,0x50 ,0xbe ,0x3b ,0x04 ,0xca ,0x36
};

uint8_t encrypted_ecb_efuse_key1[] = {
    0x2e ,0x47 ,0x72 ,0xba ,0x4d ,0x05 ,0x06 ,0x48 ,0xb2 ,0x44 ,0x98 ,0xe1 ,0xf5 ,0xf9 ,0x4a ,0x4a ,
    0x82 ,0x26 ,0x0b ,0x95 ,0x07 ,0x57 ,0x9e ,0x41 ,0x67 ,0x8a ,0x3b ,0x7a ,0xed ,0xfa ,0xdc ,0x96
};
...
ATTR_RWDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN uint8_t efuse_key1[32] = {0};
ATTR_RWDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN uint8_t efuse_key2[32] = {0};
...
/*Step2: Log out the data during AES encryption/decryption operation including CBC and ECB modes.*/
    ...
    // TEST: Fuse Key, change key length 128 -> 256
    key.length = 32;
    printf("\r\nTEST: eFUSE KEY, key length = 256\r\n");

    hal_security_efuse_key(efuse_key1, 0);
    hal_security_efuse_key(efuse_key2, 1);
    printf("efuse_key1:");
    aes_result_dump(efuse_key1, 32);
    printf("efuse_key2:");
    aes_result_dump(efuse_key2, 32);

    hal_aes_cbc_encrypt_ex(&encrypted_text, &plain_text, &key, aes_cbc_iv, HAL_AES_KEY_SOURCE_EFUSE1);
    printf("Encrypted data(AES CBC) by efuse_key1:");
    //aes_result_dump(encrypted_text.buffer, encrypted_text.length);
    if(memcmp(encrypted_text.buffer, encrypted_cbc_efuse_key0, encrypted_text.length)) {
        printf("AES CBC encryption fail\r\n");
    }
    else {
        printf("AES CBC encryption success\r\n");
    ...
    hal_aes_cbc_decrypt_ex(&decrypted_text, &encrypted_text, &key, aes_cbc_iv, HAL_AES_KEY_SOURCE_EFUSE1);
    printf("Decrypted data(AES CBC) by efuse_key1:");
    //aes_result_dump(decrypted_text.buffer, decrypted_text.length);
    if(memcmp(decrypted_text.buffer, plain, decrypted_text.length)) {
        printf("AES CBC decryption fail\r\n");
    }
    ...
    hal_aes_ecb_encrypt_ex(&encrypted_text, &plain_text, &key, HAL_AES_KEY_SOURCE_EFUSE1);
    printf("Encrypted data(AES ECB) by efuse_key1:");
    //aes_result_dump(encrypted_text.buffer, encrypted_text.length);
    if(memcmp(encrypted_text.buffer, encrypted_ecb_efuse_key0, encrypted_text.length)) {
        printf("AES ECB encryption fail\r\n");
    }
    else {
        printf("AES ECB encryption success\r\n");
    ...
    hal_aes_ecb_decrypt_ex(&decrypted_text, &encrypted_text, &key, HAL_AES_KEY_SOURCE_EFUSE1);
    printf("Decrypted data(AES ECB) by efuse_key1:");
    //aes_result_dump(decrypted_text.buffer, decrypted_text.length);
    if(memcmp(decrypted_text.buffer, plain, decrypted_text.length)) {
        printf("AES ECB decryption fail\r\n");
    }
    ...
    hal_aes_cbc_encrypt_ex(&encrypted_text, &plain_text, &key, aes_cbc_iv, HAL_AES_KEY_SOURCE_EFUSE2);
    printf("Encrypted data(AES CBC) by efuse_key2:");
    //aes_result_dump(encrypted_text.buffer, encrypted_text.length);
    if(memcmp(encrypted_text.buffer, encrypted_cbc_efuse_key1, encrypted_text.length)) {
        printf("AES CBC encryption fail\r\n");
    }
    else {
        printf("AES CBC encryption success\r\n");
    ...
    hal_aes_cbc_decrypt_ex(&decrypted_text, &encrypted_text, &key, aes_cbc_iv, HAL_AES_KEY_SOURCE_EFUSE2);
    printf("Decrypted data(AES CBC) by efuse_key2:");
    //aes_result_dump(decrypted_text.buffer, decrypted_text.length);
    if(memcmp(decrypted_text.buffer, plain, decrypted_text.length)) {
        printf("AES CBC decryption fail\r\n");
    }
    ...
    hal_aes_ecb_encrypt_ex(&encrypted_text, &plain_text, &key, HAL_AES_KEY_SOURCE_EFUSE2);
    printf("Encrypted data(AES ECB) by efuse_key2:");
    //aes_result_dump(encrypted_text.buffer, encrypted_text.length);
    if(memcmp(encrypted_text.buffer, encrypted_ecb_efuse_key1, encrypted_text.length)) {
        printf("AES ECB encryption fail\r\n");
    }
    else {
        printf("AES ECB encryption success\r\n");
    ...
    hal_aes_ecb_decrypt_ex(&decrypted_text, &encrypted_text, &key, HAL_AES_KEY_SOURCE_EFUSE2);
    printf("Decrypted data(AES ECB) by efuse_key2:");
    //aes_result_dump(decrypted_text.buffer, decrypted_text.length);
    if(memcmp(decrypted_text.buffer, plain, decrypted_text.length)) {
        printf("AES ECB decryption fail\r\n");
    }
