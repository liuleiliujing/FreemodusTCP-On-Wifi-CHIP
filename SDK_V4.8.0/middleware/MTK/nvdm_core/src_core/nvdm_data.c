...
{
    NVDM_LOG_INFO("data item header info show below:");
    NVDM_LOG_INFO("status: 0x%02x", header->status);
    NVDM_LOG_INFO("pnum: %d", header->pnum);
    NVDM_LOG_INFO("offset: 0x%04x", header->offset);
    NVDM_LOG_INFO("sequence_number: %d", header->sequence_number);
    NVDM_LOG_INFO("group_name_size: %d", header->group_name_size);
    NVDM_LOG_INFO("data_item_name_size: %d", header->data_item_name_size);
    NVDM_LOG_INFO("value_size: %d", header->value_size);
    NVDM_LOG_INFO("index: %d", header->index);
    NVDM_LOG_INFO("type: %d", header->type);
    NVDM_LOG_INFO("hash_name: 0x%08x", header->hash_name);
}
...
static int32_t search_data_item_by_name(const char *group_name, const char *data_item_name, uint32_t *hasename)
{
    ...
    NVDM_LOG_INFO("hashname = 0x%08x", hash);
                                  ...
                                  uint32_t *size)
{
    ...
    NVDM_LOG_INFO("nvdm_read_data_item: begin to read");
                                   ...
                                   uint32_t size)
{
    ...
    NVDM_LOG_INFO("nvdm_write_data_item: begin to write");
    ...
    NVDM_LOG_INFO("find_data_item_by_hashname return %d", index);
    if (index < 0) {
        append = 1;
    ...
    if (ret == false) {
        NVDM_LOG_INFO("peb free space is not enough\n");
        nvdm_port_mutex_give();
    ...
    if (append) {
        if (g_sum_data_item_headers >= g_total_data_item_count) {
            NVDM_LOG_NOTICE("too many data items in nvdm region\n");
            return NVDM_STATUS_ERROR;
        }
        NVDM_LOG_INFO("new data item append");
        ...
        NVDM_LOG_INFO("old data item overwrite");
        p_data_item_header = &g_data_item_headers[index];
        p_data_item_header->sequence_number = ++g_write_sequence_number;
...
nvdm_status_t nvdm_query_data_item_length(const char *group_name, const char *data_item_name, uint32_t *size)
{
    ...
    NVDM_LOG_INFO("nvdm_query_data_item_length: begin to query");
...
void data_item_scan(int32_t pnum)
{
    ...
    NVDM_LOG_INFO("scanning pnum(%d) to analysis data item info", pnum);
    offset = 0;
    peb_drity = 0;
    peb_valid = 0;
    ...
    while (offset < (g_nvdm_peb_size - PEB_HEADER_SIZE - DATA_ITEM_HEADER_SIZE)) {
        ...
        switch (data_item_header.status) {
            ...
            case DATA_ITEM_STATUS_DELETE:
                ...
                continue;
            default:
                NVDM_LOG_ERROR("pnum=%d, offset=0x%x", pnum, offset);
                return;
        }

        if (data_item_header.index >= g_total_data_item_count) {
            NVDM_LOG_ERROR("Detect index of data item with out of range, max = %d, curr = %d", g_total_data_item_count, data_item_header.index);
            return;
        }
        ...
        if (checksum1 != checksum2) {
            NVDM_LOG_INFO("detect checksum error\n");
            peb_drity += size;
            peb_add_drity(pnum, size);
            continue;
        }
        ...
        if (g_data_item_headers[data_item_header.index].sequence_number == MAX_WRITE_SEQUENCE_NUMBER) {
            ...
            g_sum_data_item_headers++;
            if (g_sum_data_item_headers > g_total_data_item_count) {
                NVDM_LOG_ERROR("too many data items in nvdm region\n");
                return;
            }
            peb_valid += size;
        } else {
            /* we found it before, so compare sequence number of them
                    ...
                    */
            NVDM_LOG_INFO("detect two valid copy of data item");
            NVDM_LOG_INFO("copy1(pnum=%d, offset=0x%04x), copy2(pnum=%d, offset=0x%04x)\n",
            ...
            if ((abnormal_data_item > 0) ||
                ...
                NVDM_LOG_ERROR("abnormal_data_item = %d", abnormal_data_item);
                return;
            }
...
void data_item_init(void)
{
    ...
    if (g_max_data_item_size > MAX_DATA_ITEM_SIZE) {
        NVDM_LOG_ERROR("Max size of data item must less than or equal to 2048 bytes");
        return;
    }
    ...
    if (g_data_item_headers == NULL) {
        NVDM_LOG_ERROR("alloc data_item_headers fail");
        return;
    }
            ...
            case DATA_ITEM_STATUS_VALID:
                if (g_data_item_headers[data_item_header.index].sequence_number != data_item_header.sequence_number) {
                    /* find old copy, this should not happen,
                                 ...
                                 */
                    NVDM_LOG_ERROR("old_src_pnum=%d, old_pos=0x%x, new_src_pnum=%d, new_pos=0x%x",
                                   src_pnum, pos,
                                   g_data_item_headers[data_item_header.index].pnum,
                                   g_data_item_headers[data_item_header.index].offset);
                    return 0;
                ...
                break;
            default:
                NVDM_LOG_ERROR("src_pnum=%d, pos=0x%x", src_pnum, pos);
                return 0;
        }
    }
