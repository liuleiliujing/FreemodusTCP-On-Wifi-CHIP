...
static void peb_print_info(void)
{
    ...
    NVDM_LOG_INFO("region info show below:");
    NVDM_LOG_INFO("peb    free    dirty    erase_count    is_reserved");
    for (i = 0; i < g_nvdm_peb_count; i++) {
        NVDM_LOG_INFO("%d     %d     %d     %d      %d",
                      i,
                      g_pebs_info[i].free,
                      g_pebs_info[i].dirty,
    ...
    NVDM_LOG_INFO("g_valid_data_size = %d", g_valid_data_size);
}
static void peb_header_print_info(uint32_t peb_index, peb_header_t *peb_hdr)
{
    NVDM_LOG_INFO("peb header(%d) info show below:", peb_index);
    NVDM_LOG_INFO("magic: %08x", peb_hdr->magic);
    NVDM_LOG_INFO("erase_count: %08x", peb_hdr->erase_count);
    NVDM_LOG_INFO("status: %02x", peb_hdr->status);
    NVDM_LOG_INFO("peb_reserved: %02x", peb_hdr->peb_reserved);
    NVDM_LOG_INFO("version: %02x", peb_hdr->version);
}
...
void peb_write_data(int32_t pnum, int32_t offset, const uint8_t *buf, int32_t len)
{
    ...
    if (offset >= g_nvdm_peb_size - PEB_HEADER_SIZE) {
        NVDM_LOG_ERROR("offset=0x%x", offset);
        return;
    }
    if (len > g_nvdm_peb_size - PEB_HEADER_SIZE) {
        NVDM_LOG_ERROR("len=%d", len);
        return;
    }
    ...
    if (ret == false) {
        NVDM_LOG_ERROR("magic=0x%x, erase_count=0x%x, status=0x%x, peb_reserved=0x%x",
...
static void relocate_pebs(int32_t *source_pebs, int32_t source_peb_count)
{
    ...
    if (total_valid_data == 0) {
        NVDM_LOG_INFO("found no valid data in reclaiming pebs when relocate_pebs()");
        for (i = 0; i < source_peb_count; i++) {
    ...
    if (target_peb >= g_nvdm_peb_count) {
        NVDM_LOG_ERROR("target_peb=%d", target_peb);
        return;
    }
    NVDM_LOG_INFO("found a target peb(%d) for reclaiming", target_peb);
...
static void garbage_reclaim_pebs(int32_t found_blocks, int32_t *peb_list)
{
    ...
    do {
        ...
        for (i = merge_start; i < merge_start + merge_cnt; i++) {
            NVDM_LOG_INFO("merge peb %d", peb_list[i]);
        }
...
static void garbage_collection_peb(void)
{
    ...
    NVDM_LOG_NOTICE("start garbage collection!!!");

    peb_print_info();

    peb_list = nvdm_port_malloc(g_nvdm_peb_count * sizeof(int32_t));
    if (peb_list == NULL) {
        NVDM_LOG_ERROR("peb_list alloc fail");
        return;
    }
    ...
    NVDM_LOG_INFO("non_reserved_pebs = %d", non_reserved_pebs);
    NVDM_LOG_INFO("mean_erase_count = %d", mean_erase_count);
    ...
    if (found_blocks) {
        NVDM_LOG_INFO("reclaim blocks select by erase count = %d", found_blocks);
        NVDM_LOG_INFO("reclaim peb_list(no-sort): ");
        for (i = 0; i < found_blocks; i++) {
            NVDM_LOG_INFO("%d", peb_list[i]);
        }
        for (i = 0; i < found_blocks; i++) {
        ...
        NVDM_LOG_INFO("reclaim peb_list(sort): ");
        for (i = 0; i < found_blocks; i++) {
            NVDM_LOG_INFO("%d", peb_list[i]);
        }
    }
    ...
    if (found_blocks == 0) {
        ...
        NVDM_LOG_INFO("reclaim blocks select by valid size = %d", found_blocks);
        NVDM_LOG_INFO("reclaim peb_list(no-sort): ");
        for (i = 0; i < found_blocks; i++) {
            NVDM_LOG_INFO("%d", peb_list[i]);
        }
...
static int32_t find_free_peb(int32_t size)
{
    ...
    NVDM_LOG_INFO("find_free_peb: target_peb = %d, reserved_peb = %d, reserved_peb_cnt = %d\n", target_peb, reserved_peb, reserved_peb_cnt);
...
static uint32_t calculate_total_avail_space(void)
{
    ...
    NVDM_LOG_INFO("total avail space = %d\n", max_reasonable_size);
...
{
    NVDM_LOG_INFO("space_is_enough: g_valid_data_size = %d, new add size = %d\n", g_valid_data_size, size);
...
static void peb_scan(void)
{
    ...
    if (reclaiming_peb == NULL) {
        NVDM_LOG_ERROR("reclaiming_peb alloc fail");
        return;
    }
    ...
    NVDM_LOG_INFO("scan and verify peb headers");
    reclaim_idx = 0;
    transfering_peb = -1;
    transfered_peb = -1;
    for (i = 0; i < g_nvdm_peb_count; i++) {
        peb_read_header(i, &peb_hdr);
        NVDM_LOG_INFO("before verify peb header");
        ...
        switch (peb_status) {
            ...
            case PEB_STATUS_RECLAIMING:
                ...
                if (ret == false) {
                    NVDM_LOG_ERROR("peb_header validate fail, pnum=%d", i);
                ...
                if (peb_status == PEB_STATUS_TRANSFERING) {
                    if (transfering_peb >= 0) {
                        NVDM_LOG_ERROR("find more than one transfering peb, frist=%d, second=%d", transfering_peb, i);
                        nvdm_port_free(reclaiming_peb);
                        return;
                    }
                    transfering_peb = i;
                    ...
                    if (transfered_peb >= 0) {
                        NVDM_LOG_ERROR("find more than one transfered peb, frist=%d, second=%d", transfered_peb, i);
                        nvdm_port_free(reclaiming_peb);
                        return;
                    }
                    transfered_peb = i;
        ...
        NVDM_LOG_INFO("after verify peb header");
        peb_read_header(i, &peb_hdr);
        peb_header_print_info(i, &peb_hdr);
    }
    peb_print_info();
    NVDM_LOG_INFO("transfering_peb = %d", transfering_peb);
    NVDM_LOG_INFO("transfered_peb = %d", transfered_peb);
    for (i = 0; i < reclaim_idx; i++) {
        NVDM_LOG_INFO("reclaiming_peb[%d] = %d", i, reclaiming_peb[i]);
    }

    /* update erase count for unknown pebs */
    NVDM_LOG_INFO("update erase count for unknown pebs");
    ...
    NVDM_LOG_INFO("scan all non-reserved pebs including reclaiming pebs and transfering peb");
    for (i = 0; i < g_nvdm_peb_count; i++) {
    ...
    if ((reclaim_idx > 0) && (transfered_peb < 0)) {
        /* when power-off last time, data transfering is going on.
             ...
             */
        NVDM_LOG_INFO("found a peb in transfering status");
        if (transfering_peb >= 0) {
        ...
        /* when power-off last time, data transfer is complete,
             ...
             */
        NVDM_LOG_INFO("found a peb in transfered status");
        for (i = 0; i < reclaim_idx; i++) {
        ...
        if ((reclaim_idx > 0) || (transfered_peb >= 0) || (transfering_peb >= 0)) {
            NVDM_LOG_ERROR("reclaim_idx=%d, transfered_peb=%d, transfering_peb=%d",
    ...
    nvdm_port_free(reclaiming_peb);
    NVDM_LOG_INFO("calculate total valid data size");
    g_valid_data_size = 0;
    for (i = 0; i < g_nvdm_peb_count; i++) {
...
nvdm_status_t nvdm_init(void)
{
    ...
    if (g_nvdm_peb_count < 2) {
        NVDM_LOG_ERROR("Count of PEB for NVDM region must greater than or equal to 2");
        return NVDM_STATUS_ERROR;
    }
    ...
    if (g_pebs_info == NULL) {
        NVDM_LOG_ERROR("alloc peb_info fail");
        return NVDM_STATUS_ERROR;
    }
    ...
    NVDM_LOG_INFO("nvdm init finished\n");
