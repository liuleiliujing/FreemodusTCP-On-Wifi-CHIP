...
#endif
{
    ...
    if(size_memory_pool > sizeof(mp3_codec_decode)){
        LOGE("MP3 codec memory size error!!! memory_pool size(%d) > mp3_codec_decode(%d)\n", size_memory_pool, sizeof(mp3_codec_decode));
...
static void mp3_codec_task_create(mp3_codec_media_handle_t *handle)
{
    if (mp3_handle_ptr_to_task_handle(handle) ==  NULL) {
        ...
        LOGI("[MP3 Codec] create mp3 task(0x%x)\r\n", mp3_handle_ptr_to_task_handle(handle));
    }
}
        ...
        case HAL_AUDIO_EVENT_DATA_REQUEST:
            ...
            for (loop_idx = 0; loop_idx < 2; loop_idx++) {
                ...
                if (stream_out_pcm_buffer_data_count > 0) {
                    ...
                    uint32_t curr_cnt = 0;
                    hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &curr_cnt);
                    LOGI("[MP3 Codec] curr_cnt=%d, write stream_out_pcm_buff. out_buf_ptr = 0x%x, stream_out_amount =%d \r\n", (curr_cnt / 1000), out_buf_ptr, stream_out_amount);
...
static void mp3_codec_deocde_hisr_handler(void *data)
{
    ...
    if (1 == internal_handle->media_bitstream_end) {
        LOGI("[MP3 Codec][Race condition] hisr(%d): handle->handler(handle, MP3_CODEC_MEDIA_BITSTREAM_END);\r\n", (curr_cnt / 1000));
...
static void mp3_codec_play_avm(mp3_codec_media_handle_t *handle)
{
    ...
    audio_dsp_file_pcm_param_t temp_param;
    void *p_param_share;
    ...
    LOGI("[MP3]mp3_codec_play_avm SR(%d) ch(%d)\r\n",internal_handle->sampling_rate, internal_handle->channel_number);
    ...
    open_param.stream_out_param.afe.audio_device    = HAL_AB155X_AUDIO_DEVICE_DAC_DUAL; //HAL_AUDIO_DEVICE_HEADSET;
