...
static void mp3_dsp_codec_get_id3v2_info_and_skip(mp3_codec_media_handle_t *handle, uint32_t file_size)
{
    ...
    while (1) {
        ...
        if (mp3_dsp_codec_get_bytes_from_share_buffer(handle, temp_buffer, want_get_bytes, 0) != want_get_bytes) {
            LOGI("[MP3 Codec]mp3_dsp_codec_get_id3v2_info_and_skip: share buffer data amount less than ID3v2 header length\n");
            return;    // just return
        }
...
static mp3_codec_function_return_state_t mp3_dsp_codec_request_data_to_share_buffer(mp3_codec_media_handle_t *handle)
{
    ...
    if (!handle->waiting) {
#if defined(MTK_AUDIO_MIXER_SUPPORT)
        LOGI("[MP3 Codec] role%d request data: share = %d, pcm = %d\r\n", handle->mixer_track_role, mp3_dsp_codec_get_share_buffer_data_count(handle), ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff));
#else
        LOGI("[MP3 Codec] request data: share = %d, pcm = %d\r\n", mp3_dsp_codec_get_share_buffer_data_count(handle), ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff));
#endif
