...
void atci_serial_port_data_callback(serial_port_dev_t device, serial_port_callback_event_t event, void *parameter)
{
    ...
    switch(event) {
        case SERIAL_PORT_EVENT_READY_TO_READ: {
            LOGI("%s, read event\r\n",ATCI_SERIAL_PORT_DATA_CALLBACK);
        ...
        case SERIAL_PORT_EVENT_USB_DISCONNECTION: {
            LOGI("%s, usb disconnection event\r\n",ATCI_SERIAL_PORT_DATA_CALLBACK);
            //atci_deinit_ext();
...
atci_status_t atci_serial_port_init(serial_port_dev_t port)
{
    ...
    LOGI("%s enter,select port:%d \r\n", ATCI_SERIAL_PORT_INIT, port);

    if (g_atci_local_startup_flag == 1 || g_atci_local_startup_flag == 2) {
        LOGW("%s already inited \r\n",ATCI_SERIAL_PORT_INIT);
    ...
    LOGW("%s serial_port_open ret=%d \r\n", ATCI_SERIAL_PORT_INIT, status);
...
uint32_t atci_serial_port_read_data(serial_port_handle_t handle, uint8_t *buf, uint32_t buf_len)
{
    ...
    LOGI("%s serial_port_control read ,status=%d\r\n",ATCI_SERIAL_PORT_READ_DATA,status);
    if (status != SERIAL_PORT_STATUS_OK) {
        LOGE("%s serial_port_control read fail \r\n",ATCI_SERIAL_PORT_READ_DATA);
        return data_len;
    }
...
{
    LOGI("%s enter:%d\r\n", ATCI_SERIAL_PORT_SWITCH, device);

    serial_port_status_t status;
    serial_port_open_para_t serial_port_open_para;

    if(device == g_serial_port) {
        LOGW("%s switch port == current port \r\n",ATCI_SERIAL_PORT_SWITCH);
        return;
    }
    ...
    LOGI("%s serial_port_open ret=%d \r\n", ATCI_SERIAL_PORT_SWITCH, status);
    ...
    switch (status) {
        ...
        case SERIAL_PORT_STATUS_OK:
            ...
            LOGI("%s success atci_status:%d \r\n", ATCI_SERIAL_PORT_SWITCH, g_atci_local_startup_flag);
            break;
...
//       case SERIAL_PORT_STATUS_PORT_PENDING: /* for bt spp */
        ...
        default :
            /* not recoverable */
            g_atci_local_startup_flag = 1;
            LOGW("%s fail because the port is busy \r\n",ATCI_SERIAL_PORT_SWITCH);
            break;
    }
...
void atci_uart_irq(hal_uart_callback_event_t event, void *parameter)
{
    ...
    if (g_atci_port_handle >= HAL_UART_MAX) {
        ...
        LOGW("%s, invaild uart port:%d\r\n", ATCI_UART_IRQ, g_atci_port_handle);
        return;
    }
...
atci_status_t atci_local_init(void)
{
    ...
    LOGI("%s enter flag:%d \r\n", ATCI_LOCAL_INIT, g_atci_local_startup_flag);
...
atci_status_t atci_send_data_int(uint8_t *data, uint32_t data_len)
{
    ...
    LOGI("%s send data len:%x\r\n", ATCI_SEND_DATA_INT, sent_len);
    return ret;
}
...
atci_status_t atci_send_data(uint8_t *data, uint32_t data_len)
{
    ...
    LOGI("%s enter, cache_left_len:%x \r\n", ATCI_SEND_DATA, g_send_data_cache_left_len);
    if(atci_vaild_port_check(g_atci_port_handle) == ATCI_STATUS_ERROR) {
        LOGE("%s fail,because g_atci_port_handle is invalid:%d \r\n", ATCI_SEND_DATA, g_atci_port_handle);
        return ATCI_STATUS_ERROR;
    }
    ...
    LOGI("%s sending data(len:%x), sent data(len:%x)\r\n", ATCI_SEND_DATA, send_len, sent_len);
    ...
    if(sent_len >= send_len) {
        ...
        if(atci_input_commad_in_processing == 3) {
            atci_input_commad_in_processing = 0;
            LOGI("%s send AT cmd response done\r\n",ATCI_SEND_DATA);
        } else {
            LOGI("%s send data done\r\n",ATCI_SEND_DATA);
        }
        ...
        if(g_send_data_cache_len == 0) {
            ...
            if(g_send_data_cache == NULL) {
                ...
                if(g_send_data_cache == NULL) {
                    LOGW("%s alloc memory fail, drop the cached data(data_len:%x)\r\n",ATCI_SEND_DATA, g_send_data_cache_left_len);
                    g_send_data_cache_len = 0;
                    g_send_data_cache_left_len = 0;
                    return ATCI_STATUS_ERROR;
                }
...
atci_status_t atci_read_data(void)
{
    ...
    LOGI("%s, enter,g_atci_port_handle:%x\r\n", ATCI_READ_DATA, g_atci_port_handle);
    if(atci_vaild_port_check(g_atci_port_handle) == ATCI_STATUS_ERROR) {
        return ATCI_STATUS_ERROR;
    }
    ...
    if(input_data == NULL) {
        ...
        if(input_data == NULL) {
            LOGW("%s alloc mem fail.drop data \r\n",ATCI_READ_DATA);
            return ATCI_STATUS_ERROR;
        }
    }
    ...
    if(g_input_command == NULL) {
        ...
        LOGW("%s alloc mem to get data from port\r\n",ATCI_READ_DATA);
        if(g_input_command == NULL) {
            atci_mem_free(input_data);
            LOGW("%s alloc mem fail\r\n",ATCI_READ_DATA);
            return ATCI_STATUS_ERROR;
        }
    ...
    do {
        ...
        if((read_data_len > 0) && (atci_input_commad_in_processing == 0)) {
            ...
            if((read_data_len + g_input_command->input_len) < ATCI_UART_RX_FIFO_BUFFER_SIZE) {
                ...
                LOGW("%s atci_port_read_data len is bigger than %d\r\n", ATCI_READ_DATA, ATCI_UART_RX_FIFO_BUFFER_SIZE);
            }
        }
        memset(input_data, 0, input_data_len);
    } while(read_data_len);
    ...
    if (atci_input_commad_in_processing != 0) {
        ...
        LOGW("%s drop this cmd, need wait the response of previous AT cmd \r\n",ATCI_READ_DATA);
        return ATCI_STATUS_ERROR;
    }
    ...
    if ((g_input_command->input_len) < ATCI_UART_RX_FIFO_BUFFER_SIZE) {
        ...
        g_input_command = NULL;
        atci_send_data((uint8_t *)"ERROR\r\n", strlen("ERROR\r\n"));
        LOGW("%s fail, input AT cmd length is larger than %d \r\n", ATCI_READ_DATA, ATCI_UART_RX_FIFO_BUFFER_SIZE);
        return ATCI_STATUS_ERROR;
    }
    ...
    if(find_valid_command == 1) {
        ...
        if (('A' == g_input_command->input_buf[0] && 'T' == g_input_command->input_buf[1]) ||
            ...
            g_input_command = NULL;
            atci_send_data((uint8_t *)"ERROR\r\n", strlen("ERROR\r\n"));
            LOGW("%s fail, receive data is not AT cmd type, drop this cmd! \r\n",ATCI_READ_DATA);
            return ATCI_STATUS_ERROR;
        }
    }
    return ret;
...
void atci_processing(void)
{
    ...
    while (1) {
        ...
        if (common_queue_msg_num > 0) {
            LOGI("%s commad_in_processing(%x), input_queue_msg_num(%d)\r\n", ATCI_PROCESSING, atci_input_commad_in_processing, common_queue_msg_num);

            atci_queue_receive_no_wait(g_atci_input_command_queue,&msg_queue_data);

            LOGI("%s msg_id(%x)\\r\n", ATCI_PROCESSING, msg_queue_data.msg_id);
        ...
        if(cached_queue_msg_num >0) {
            LOGI("%s cached_queue_msg_num(%d)\r\n", ATCI_PROCESSING, cached_queue_msg_num);
        }
        ...
        while (cached_queue_msg_num > 0) {
            ...
            if (msg_queue_data.msg_id == MSG_ID_ATCI_LOCAL_WRITE_CMD_IND) {
                ...
                LOGI("%s g_send_data_cache_left_len(%x)\r\n", ATCI_PROCESSING, g_send_data_cache_left_len);
                if (g_send_data_cache_left_len > 0) {
        ...
        if(notify_queue_msg_num >0) {
            LOGI("%s notify_queue_msg_num(%d)\r\n", ATCI_PROCESSING, notify_queue_msg_num);
        }
        ...
        while ((g_send_data_cache_left_len == 0 ) && (notify_queue_msg_num > 0)) {
            ...
            LOGI("%s handle AT send cmd done\r\n",ATCI_PROCESSING);
        ...
        if(atcmd_switch_racecmd_flag)
        {
            ...
            LOGI("atci_processing() finish, at cmd switch to race cmd.\r\n");
            //break;
        }
    }
}
...
atci_status_t atci_data_processing_for_deinit(void)
{
    ...
    if(cached_queue_msg_num >0) {
        LOGI("%s cached_queue_msg_num(%d)\r\n", ATCI_DATA_PROCESSING_FOR_DEINIT, cached_queue_msg_num);
    }
    ...
    notify_queue_msg_num = atci_queue_get_item_num(g_atci_resonse_command_queue);
    if(notify_queue_msg_num >0) {
        LOGI("%s notify_queue_msg_num(%d)\r\n", ATCI_DATA_PROCESSING_FOR_DEINIT, notify_queue_msg_num);
    }
...
atci_status_t atci_deinit(hal_uart_port_t port)
{
    ...
    LOGI("%s enter (flag:%d)\r\n", ATCI_DEINIT, g_atci_local_startup_flag);
...
static void atci_start_sleep_lock_timer(void)
{
    if(atci_uart_rx_sleep_handle == 0xFF)
    {
        ...
        LOGI("create atci sleep lock handler success %d \r\n", atci_uart_rx_sleep_handle);
    }
    if(atci_sleep_lock_timer == NULL)
    {
        ...
        if(atci_sleep_lock_timer == NULL)
        {
            ...
            LOGI("create atci sleep lock timer fail\r\n");
            return;
        }
        LOGI("create atci sleep lock timer success\r\n");
    }
