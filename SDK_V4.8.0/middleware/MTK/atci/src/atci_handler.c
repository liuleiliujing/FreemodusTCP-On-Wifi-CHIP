...
atci_status_t atci_register_handler(atci_cmd_hdlr_item_t *hdlr_table, int32_t hdlr_number)
{
    ...
    if(1) {
        ...
        if ( ret == ATCI_STATUS_OK) {
            ...
            LOGI("%s  register success \r\n", ATCI_HANDLER);
        } else {
            LOGE("%s register fail \r\n", ATCI_HANDLER);
            ret = ATCI_STATUS_REGISTRATION_FAILURE;
        }
    } else {

        LOGE("%s register fail, please call atci_init() frist \r\n", ATCI_HANDLER);
    }
...
atci_status_t atci_send_response(atci_response_t *response)
{
    ...
    if (g_atci_local_startup_flag != 1) {
        LOGW("%s fail, atci does not ready!\r\n", ATCI_SEND_RESPONSE);
        return ret;
    }
    ...
    if (notify_queue_msg_num >= ATCI_LOCAL_QUEUE_LENGTH) {
        ...
        if (ATCI_RESPONSE_FLAG_URC_FORMAT == (response->response_flag & ATCI_RESPONSE_FLAG_URC_FORMAT)) {
            LOGW("%s, atci response queue full(%d), drop this urc data!\r\n", ATCI_SEND_RESPONSE, notify_queue_msg_num);
        } else {
            ...
            LOGW("%s, atci response queue full(%d), drop this rsp data!\r\n", ATCI_SEND_RESPONSE, notify_queue_msg_num);
        }
...
static atci_status_t atci_init_hdlr_tbl_hash_value(atci_cmd_hdlr_item_t *hdlr_table, uint16_t hdlr_number)
{
    ...
    if (memcmp(atcmd_table,hdlr_table,sizeof(atci_cmd_hdlr_item_t)) == 0) {
        ...
        LOGW("atci_init_hdlr_tbl_hash_value, exclude atcmd_table success\r\n");
    } else {
...
atci_status_t atci_input_command_handler(atci_send_input_cmd_msg_t *cmd_msg)
{
    ...
    if(cmd_msg == NULL) {
        LOGW("%s input buf is null\r\n", ATCI_INPUT_COMMAND_HANDLER);
        return ATCI_STATUS_ERROR;
    }

    LOGI("%s, input_buf: %s\n", ATCI_INPUT_COMMAND_HANDLER, cmd_msg->input_buf);
