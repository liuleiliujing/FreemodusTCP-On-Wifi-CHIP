    ...
    case ATCI_CMD_MODE_EXECUTION:
        ...
        if (strncmp(parse_cmd->string_ptr, "AT+CAPID=", strlen("AT+CAPID=")) == 0) {
            ...
            if(0 == things) { //AT+CAPID=0,2   enter relay mode on uart2
                ...
                printf("Set %u to CAPID register.\r\n", (int)debug_capid);
#else
                ...
                if (status != NVDM_STATUS_OK) {
                    printf("DCXO CAPID write to NVDM failed !!\r\n");
                } else{
                    printf("CAPID %u write to NVDM succeed\r\n", (int)debug_capid);
                }
#else
                ...
                if (status != NVDM_STATUS_OK) {
                    printf("DCXO CAPID load from NVDM failed !!\r\n");
                } else{
                    printf("CAPID load from NVDM = %u\r\n", (int)result);
                }
#else
...
#if defined(MTK_CAPID_IN_NVDM_AT_COMMAND_ENABLE) && defined(MTK_NVDM_ENABLE)
            printf("Support at_command to calibration capid\r\n");
#else
            printf("Not support at_command to calibration capid\r\n");
#endif /* MTK_CAPID_IN_NVDM_AT_COMMAND_ENABLE && MTK_NVDM_ENABLE */
                resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
            }
