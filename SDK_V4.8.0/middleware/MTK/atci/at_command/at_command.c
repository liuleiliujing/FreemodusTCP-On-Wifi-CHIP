...
extern atci_status_t atci_cmd_offline_dump(atci_parse_cmd_param_t *parse_cmd);
#endif
...
#endif
#ifdef MTK_SAVE_LOG_AND_CONTEXT_DUMP_ENABLE
    {"AT+OFFLINE_DUMP", atci_cmd_offline_dump, 31615401, 29473657},
#endif
        ...
        case ATCI_CMD_MODE_EXECUTION: // rec: AT+TEST=<p1>  the handler need to parse the parameters
            ...
            if (param != NULL && strlen(param) > 0) {
                ...
                if ((param[0] == '0' || param[0] == '1')) {
                    ...
                    LOGW("AT Read mode after caculate, hash1=%x,hash2=%x",*hash1,*hash2);
                    snprintf((char *)response.response_buf, sizeof(response.response_buf), "+hash1:%x,hash2:%x\r\n", *hash1,*hash2);
                    response.response_len = strlen((char *)response.response_buf);
                    response.response_flag |= ATCI_RESPONSE_FLAG_APPEND_OK;  // ATCI will help append OK at the end of resonse buffer
                    atci_send_response(&response);
                    atci_mem_free(hash1);
