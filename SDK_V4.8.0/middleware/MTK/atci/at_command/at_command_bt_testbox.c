...
atci_status_t atci_cmd_hdlr_bt_testbox_CW_tone_readRSSI(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    case ATCI_CMD_MODE_EXECUTION:    // 
        ...
        if (strncmp(parse_cmd->string_ptr, "AT+EBBTRXRSSI=", strlen("AT+EBBTRXRSSI=")) == 0) {
            ...
            things = atoi(param1);
            LOG_I(atci_bt, "things=%d\r\n", things);
            if (0 == things) {
                p = (uint8_t *)(RSSI_BASE);
                for(length = 0;length < 8 ; length++) {
                    //LOGI("RSSI = %2x \n",*p);
                    LOG_I(atci_bt, "things=%d,current RSSI = %2x \r\n", things,*p);
                    if(length == 2) {
                        result = *p;
                    }
                    p++;
        ...
        sprintf((char *)resonse.response_buf, "+EBBTRXRSSI:%d,%d\r\n",things,result);
        resonse.response_len = strlen((char *)resonse.response_buf);
        atci_send_response(&resonse);
        break;
