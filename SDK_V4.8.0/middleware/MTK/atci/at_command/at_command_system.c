...
#define AT_COMMAND_OFFLINE_DUMP_HELP "AT+OFFLINE_DUMP=<log save time(ms)>,<assert time(s)>"
...
atci_status_t atci_cmd_offline_dump(atci_parse_cmd_param_t *parse_cmd)
{
    atci_response_t response = {{0}, 0};
    char delim[] = ",";
    char *str_log_time, *str_assert_time, *saveptr = NULL;

    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_TESTING: /* rec: AT+OFFLINE_DUMP=? */
            strcpy((char *)(response.response_buf), AT_COMMAND_OFFLINE_DUMP_HELP);
            response.response_len = strlen((char *)(response.response_buf));
            response.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
            break;
        case ATCI_CMD_MODE_EXECUTION:
            if (strstr((char *)parse_cmd->string_ptr, "AT+OFFLINE_DUMP=0") != NULL) {
                str_log_time = strtok_r(parse_cmd->string_ptr, delim, &saveptr);
                str_log_time = strtok_r(NULL, delim, &saveptr);
                if (str_log_time == NULL) {
                    response.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    break;
                }
                str_assert_time = strtok_r(NULL, delim, &saveptr);
                if (str_assert_time == NULL) {
                    response.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    break;
                }
                g_log_during_time = atoi(str_log_time);
                g_assert_during_time = atoi(str_assert_time);
