...
{
    const char *charger_type[9] = { "", "SDP", "CDP", "DCP", "SS", "IPAD2_IPAD4", "IPHONE_5V_1A", "NON-STD", "DP/DM_Floating", "UNABLE_TO_IDENTIFY", };
    ...
    for(;; )
    {
        ...
        if (charger_status) {
            printf("[BM]Battery capacity = %d(%%) ", (int)battery_management_get_battery_property(BATTERY_PROPERTY_CAPACITY));
            printf("[BM]Charger status = %d ", (int)charger_status);
            printf("[BM]Charger type = %s",charger_type[battery_management_get_battery_property(BATTERY_PROPERTY_CHARGER_TYPE)]);
            printf("[BM]Battery temperature = %d(Celsius degrees)", (int)battery_management_get_battery_property(BATTERY_PROPERTY_TEMPERATURE));
            printf("[BM]Battery voltage = %d(mV)\n",(int) battery_management_get_battery_property(BATTERY_PROPERTY_VOLTAGE));
            printf("[BM]PMIC temperature = %d(Celsius degrees)\n",(int) battery_management_get_battery_property(BATTERY_PROPERTY_PMIC_TEMPERATURE));
            printf("[BM]Charger State = %s\n", charger_state[battery_management_get_battery_property(BATTERY_PROPERTY_CHARGER_STATE)]);
        } else {
            printf("[BM]Battery capacity = %d(%%)",(int) battery_management_get_battery_property(BATTERY_PROPERTY_CAPACITY));
            printf("[BM]Charger status = %d(0:charge not exist; 1:charger exist)",(int) charger_status);
            printf("[BM]Battery temperature = %d(Celsius degrees)",(int) battery_management_get_battery_property(BATTERY_PROPERTY_TEMPERATURE));
            printf("[BM]Battery voltage = %d(mV)\n", (int)battery_management_get_battery_property(BATTERY_PROPERTY_VOLTAGE));
        }
    }
...
atci_status_t atci_cmd_hdlr_charger(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        ...
        if(strncmp(parse_cmd->string_ptr, "AT+ECHAR=SQC,", 12) == 0) {
            ...
            mid_pos = NULL;
            end_pos = NULL;
            printf("input_value1:%d, input_value2:%d\r\n", input_domain, input_value);
            result = true;
            ...
            switch (input_domain) {
                ...
                if (charger_task_handle == NULL) {
                    xTaskCreate(charger_task, "charger_task", 2048, (void*)&input_value, tskIDLE_PRIORITY, &charger_task_handle);
                }
                break;
