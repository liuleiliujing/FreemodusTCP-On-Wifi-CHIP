...
void atci_captouch_interrupt_handler(void *user_data)
{
    ...
    while (1) {
        ...
        printf("[captouch]key_data = %d, state = %d, time_stamp = %d;", (int)key_data, (int)state, (int)time_stamp);
        ...
        case ATCI_CMD_MODE_EXECUTION:
            ...
            */
            if (strncmp(param1, "AT+ECPT=CAL_KEY", strlen("AT+ECPT=CAL_KEY")) == 0) {
                ...
                nvdm_read_data_item("CAPTOUCH", "CALI", (uint8_t *)(&captouch_data), &temp);
                ...
                for (i = 0; i < 8; i++) {
                    printf("is_key=%d, crs_cap=%d, f_cap=%d\r\n", captouch_data.is_key[i], captouch_data.coarse_cap[i], captouch_data.fine_cap[i]);
                }



                nvdm_write_data_item("CAPTOUCH", "CALI", NVDM_DATA_ITEM_TYPE_STRING, (uint8_t *)(&captouch_data), sizeof(hal_captouch_nvdm_data));
                ...
                while (1) {
                    ...
                    for (i = 0; i < 8; i++) {
                        if ((temp & (1 << i)) == 0) {
                            ...
                            printf("    CH%d press: coarse_cap=%d, fine_cap=%d,avg_adc=%d,vadc=%d\r\n", \
