    ...
    case ATCI_CMD_MODE_EXECUTION: // rec: AT+SWITCH=<p1>  the handler need to parse the parameters
        ...
        if (param != NULL && strlen(param) > 0)
        {
            if (param[0] == '1' || param[0] == '2')
            {
                ...
                {
                    isSwitchRace = false;
                    LOGW("param len is short,no need to switch");
                }
                else
                {
        ...
        if (isSwitchRace) {
            ...
            {
                atci_status_t sta = atci_deinit();
                LOGW("AT Switch call atci_deinit. result=%d\r\n", sta);
            }
            ...
            if (g_atci_uart_port == g_atci_uart_port_default)
            {
                ...
                {
                    LOGI("race uart init fail...! port:%d\r\n", g_atci_uart_port);
            ...
            /*if (!race_is_task_existed())
            {
                ...
                if (ret != RACE_STATUS_OK)
                {
                    LOGI("race init fail...! port:%d", SERIAL_PORT_DEV_UART_2);
