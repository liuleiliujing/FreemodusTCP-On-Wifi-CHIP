    ...
    *read_count = 0;
    while ((i < BT_DEVICE_MANAGER_MAX_PAIRED_NUM) && (*read_count < buff_size)) {
        if (bt_device_manager_paired_device_index[i].pase_index != 0) {
            ...
            memcpy(p->name, item.name, BT_GAP_MAX_DEVICE_NAME_LENGTH);
            (*read_count)++;
            p++;
        }
        i++;
        ...
        case BT_GAP_READ_REMOTE_NAME_COMPLETE_IND:
        {
            ...
            for(i = 0; i < BT_DEVICE_MANAGER_MAX_PAIRED_NUM; i++) {
                if (memcmp(p->address, bt_device_manager_paired_device_index[i].address, sizeof(bt_bd_addr_t)) == 0) {
                    ...
                    strncpy(paired_item.name, p->name, BT_GAP_MAX_DEVICE_NAME_LENGTH);
                    bt_device_manager_store_paired_item(&paired_item, i);
                    //bt_device_manager_notify_paired_list_changed(BT_DEVICE_MANAGER_PAIRED_ADD, p->address);
                    break;
                }
            }
            break;
...
uint32_t bt_device_manager_get_complete_paired_list(bt_device_manager_paired_device_complete_infomation_t* info, uint32_t read_count)
{
    ...
    bt_device_manager_paired_device_complete_infomation_t* p = info;
    if(NULL == info) {
        bt_device_manager_report("[device manager] get complete paired list error input info pointer.");
        return 0;
    }
    while ((i < BT_DEVICE_MANAGER_MAX_PAIRED_NUM) && (info_size < read_count)) {
        if (bt_device_manager_paired_device_index[i].pase_index != 0) {
            if(0 == bt_device_manager_read_paired_item((bt_device_manager_paired_device_store_item_t *)p, i)) {
                info_size++;
                p++;
            }
        }
...
bt_status_t bt_device_manager_get_complete_paired_info(bt_bd_addr_ptr_t address, bt_device_manager_paired_device_complete_infomation_t *info)
{
    uint32_t i = 0;
    if(NULL == info) {
        bt_device_manager_report("[device manager] get complete paired info error input info pointer.");
        return BT_STATUS_FAIL;
    }

    for(i = 0; i < BT_DEVICE_MANAGER_MAX_PAIRED_NUM ; i++) {
        if (memcmp(address, bt_device_manager_paired_device_index[i].address, sizeof(bt_bd_addr_t)) == 0) {
            if(0 == bt_device_manager_read_paired_item((bt_device_manager_paired_device_store_item_t *)info, i)) {
                return BT_STATUS_SUCCESS;
            }
        }
    }
...
bt_status_t bt_device_manager_set_complete_paired_info(bt_bd_addr_ptr_t address, bt_device_manager_paired_device_complete_infomation_t *info)
{
    ...
    uint32_t i, j = 0xFF;
    memset(&item, 0, sizeof(bt_device_manager_paired_device_store_item_t));
    bt_device_manager_delete_paired_device_int(address);
    if (info->paired_key.key_type != BT_GAP_LINK_KEY_TYPE_INVAILIDE) {
         // Save the new link key, and key_type to link key database.
        for(i = 0; i < BT_DEVICE_MANAGER_MAX_PAIRED_NUM; i++) {
            if (bt_device_manager_paired_device_index[i].pase_index > 0) {
                if (bt_device_manager_paired_device_index[i].pase_index == BT_DEVICE_MANAGER_MAX_PAIRED_NUM) {
        ...
        bt_device_manager_paired_device_index[j].pase_index = 1;
        memcpy(bt_device_manager_paired_device_index[j].address, &(info->paired_key.address), sizeof(bt_bd_addr_t));
        memcpy(&(item.paired_key), &(info->paired_key), sizeof(bt_gap_link_key_notification_ind_t));
        memcpy(item.name, info->name, BT_GAP_MAX_DEVICE_NAME_LENGTH);
        bt_device_manager_store_paired_item(&item, j);
        if (delete_item.pase_index != 0) {
            bt_device_manager_notify_paired_list_changed(BT_DEVICE_MANAGER_PAIRED_LIST_FULL, delete_item.address);
        }
        bt_device_manager_notify_paired_list_changed(BT_DEVICE_MANAGER_PAIRED_ADD, address);
    } else {
        bt_device_manager_notify_paired_list_changed(BT_DEVICE_MANAGER_PAIRED_KEY_MISS, address);
    }    
#ifdef __BT_DEVICE_MANAGER_DEBUG_INFO__
    bt_device_manager_dump_bt_address((uint8_t *)address);
    bt_device_manager_dump_link_key((uint8_t *)info->paired_key.key);
#endif
    return BT_STATUS_SUCCESS;
}
