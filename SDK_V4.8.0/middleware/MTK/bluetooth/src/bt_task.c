...
#ifdef __MTK_BT_GFPS_ENABLE__
extern bt_status_t Gfps_Handle_Interrup(void);
#endif
...
static void bt_task_generate_random_address(bt_bd_addr_t addr)
{
    ...
    hal_trng_deinit();
        ...
        addr[4] = 0x90;
        addr[5] = 0x87;
        LOG_I(common, "[BT]random address [%02X:%02X:%02X:%02X:%02X:%02X]\n", addr[0],
...
#ifdef __MTK_BT_GFPS_ENABLE__
        if (BT_STATUS_SUCCESS != Gfps_Handle_Interrup()) {
            break;
        }
#endif
