...
/* Support External Flash FOTA sector. */
#ifndef MTK_RACE_BSP_FLASH_NOT_SUPPORT
...
#include "syslog.h"
#if FOTA_DEBUG_LOG_ENABLE
#define FOTA_LOG_D(fmt, ...)  LOG_I(fota_race, (fmt), ##__VA_ARGS__)
#else
#define FOTA_LOG_D(fmt, ...)
#endif
#define FOTA_LOG_I(fmt, ...)  LOG_I(fota_race, (fmt), ##__VA_ARGS__)
#define FOTA_LOG_W(fmt, ...)  LOG_W(fota_race, (fmt), ##__VA_ARGS__)
#define FOTA_LOG_E(fmt, ...)  LOG_E(fota_race, (fmt), ##__VA_ARGS__)
#endif
