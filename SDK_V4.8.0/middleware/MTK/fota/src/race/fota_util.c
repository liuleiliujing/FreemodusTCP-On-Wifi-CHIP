...
#define FOTA_READ_BUFFER_LENGTH  (256)


log_create_module(fota_race, PRINT_LEVEL_INFO);


fota_type_enum g_fota_type = FOTA_TYPE_NONE;
...
FOTA_ERRCODE fota_crc32_generate(uint32_t *crc, uint32_t data_addr, uint32_t data_length, bool is_int)
{
    ...
    int read_length = FOTA_READ_BUFFER_LENGTH;
#ifdef BL_FOTA_ENABLE
    uint8_t buffer[FOTA_READ_BUFFER_LENGTH] = {0};
#else
    uint8_t *buffer = NULL;
#endif
...
#ifndef BL_FOTA_ENABLE
    buffer = pvPortCalloc(1, FOTA_READ_BUFFER_LENGTH);
    if (!buffer)
    {
    ...
    while (data_length > 0)
    {
        if (data_length < FOTA_READ_BUFFER_LENGTH)
        {
...
static FOTA_ERRCODE fota_parse_version_in_header(uint8_t *buffer, uint8_t buf_size)
{
	...
	FOTA_LOG_I("start parser header");
#ifndef BL_FOTA_ENABLE
    if (!buffer || !buf_size)
    {
	...
	if (fota_flash_get_fota_partition_info(&flash_type, &fota_partition_start_address, &length) != FOTA_ERRCODE_SUCCESS) {
			FOTA_LOG_E("Read record length in Bisc info failed.");
			return FOTA_ERRCODE_FAIL;
	}
    ...
    do
    {
        ...
        FOTA_LOG_I("cur_addr:%lx, tlv_type:%x, tlv_length:%d", curr_addr, tlv_type, tlv_length);
        ...
        if (FOTA_HEADER_TLV_VERSION_INFO == tlv_type) {
            ...
            FOTA_LOG_I("read finish tlv_length:%x", tlv_length);
    		for (int i = 0 ; i < tlv_length; i++) {
    			FOTA_LOG_I("read_data: i=%d, ver = %x ", i, buffer[i]);
    		}
            return FOTA_ERRCODE_SUCCESS;
        }
...
void fota_type_set(fota_type_enum fota_type)
{
    g_fota_type = fota_type;
}


fota_type_enum fota_type_get(void)
{
    return g_fota_type;
}
