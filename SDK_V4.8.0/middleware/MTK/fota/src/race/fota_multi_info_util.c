...
static FOTA_ERRCODE fota_multi_info_sector_erase(void)
{
    ...
    FOTA_LOG_I("Before erase start_addr:%x", start_addr);
    return fota_flash_erase(start_addr,
                           HAL_FLASH_BLOCK_4K,
                           InternalFlash == storage_type);
    FOTA_LOG_I("End erase start_addr:%x", start_addr);
}


/* Even if version write back failed, return FOTA_ERRCODE_SUCCESS if erase succeeds. */
FOTA_ERRCODE fota_multi_info_sector_reset(void)
{
    int32_t ret = FOTA_ERRCODE_FAIL;
    uint8_t version[FOTA_VERSION_MAX_SIZE] = {0};
    int32_t version_len = -1;
...
#ifdef MTK_NVDM_ENABLE
        /* Store the version into the nvdm if the version is valid and is not '\0' */
        nvdm_write_data_item(FOTA_NVDM_GROUP_NAME,
                             FOTA_NVDM_DATA_ITEM_VERSION,
                             NVDM_DATA_ITEM_TYPE_STRING,
                             (const uint8_t *)version,
                             strlen((const char *)version));
#endif
    }
    else
    {
        /* Try to read version from NVDM */
#ifdef MTK_NVDM_ENABLE
        ret = nvdm_read_data_item(FOTA_NVDM_GROUP_NAME,
                                  FOTA_NVDM_DATA_ITEM_VERSION,
                                  version,
                                  &version_len);
    ...
    FOTA_LOG_I("version:%s, version_len:%d, version[0]:%x, ret:%d", version, version_len, version[0], ret);
    ...
    if (version_len >= 0)
    {
        fota_version_write(version, version_len);
    }
                                                    ...
                                                    uint16_t *offset)
{
    ...
    {        
        ...
        case FOTA_MULTI_INFO_TYPE_NULL:
        {
            *offset = FOTA_MULTI_INFO_NULL_ADDR_OFFSET;
            break;
        }
                                                   ...
                                                   fota_multi_info_type_enum info_type)
{
    ...
    *addr += offset;
    ...
    FOTA_LOG_I("%s, addr. %x", __FUNCTION__, *addr);
    return FOTA_ERRCODE_SUCCESS;
}
                                                 ...
                                                 fota_multi_info_type_enum info_type)
{
    ...
    if (FOTA_MULTI_INFO_TYPE_MAX <= info_type + 1)
    {
        /* If it's the last type of info, get the start address of the flash sector just following the FOTA sector. */
...
FOTA_ERRCODE fota_multi_info_init(fota_multi_info_type_enum info_type)
{
    ...
    FOTA_LOG_I("info_type:%d, info->is_init:%d",
               info_type,
               info->is_init);
    ...
    FOTA_LOG_I("info_type:%d start_addr:%x last_record_addr:%x data_num:%d history_record_count:%d",
               info_type,
               start_addr,
                                      ...
                                      uint8_t record_len)
{
    ...
    FOTA_LOG_I("info_type:%d, record_data:%x, record_len:%d",
    ...
    FOTA_LOG_I("ret:%d, record_data:%x %x",
               ret,
               record_data[0],
               record_data[1]);
    return ret;
                                        ...
                                        uint8_t record_len)
{
    ...
    FOTA_LOG_I("buffer:%x, record_data:%x, record_len:%d",
    ...
    /* Erase the last 4K and rewrite the last record for each info type. */
    if (FOTA_ERRCODE_OUT_OF_RANGE == ret_val)
    {
        uint16_t state = 0;
        bool upgrade_flag_set = FALSE;
#ifndef BL_FOTA_ENABLE
        uint8_t integrity_res = 0;
#endif
        /* Erase the multi info sector and re-write all info */
        FOTA_LOG_W("Record is full. info_type:%d", info_type);
...
#ifndef BL_FOTA_ENABLE
        ret = fota_dl_integrity_res_write(integrity_res);
        if (FOTA_ERRCODE_SUCCESS != ret)
        {
            ret_val = ret;
        }
    ...
    {
        // TODO: if last4k is erased, address is not right any more.
        // TODO: write current value directly instead of last record before erase.
