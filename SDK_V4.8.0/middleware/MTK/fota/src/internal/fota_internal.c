...
#else
#include "bl_common.h"
#define FOTA_INT_PRINT_I(fmt,...) BL_PRINT(LOG_DEBUG, (fmt), ##__VA_ARGS__)
#define FOTA_INT_PRINT_W(fmt,...) BL_PRINT(LOG_DEBUG, (fmt), ##__VA_ARGS__)
#define FOTA_INT_PRINT_E(fmt,...) BL_PRINT(LOG_DEBUG, (fmt), ##__VA_ARGS__)
#endif
...
{
#ifdef MTK_FOTA_SB_RSA_CHECK_ENABLE
    fota_status_t status = FOTA_STATUS_OK;
#if defined(MTK_SEC_FLASH_SCRAMBLE_ENABLE)
