...
typedef enum _manage_tcp_link_event
{
	MANAGE_TCP_DOWN=0,			/*连接AP失败*/
	MANAGE_TCP_LINK,			/*连接AP成功*/	
}manage_tcp_link_event;

typedef enum _manage_tcp_task_event
{
	MANAGE_TCP_TASK_DOWN=0,			/*私有TCP任务未创建*/
	MANAGE_TCP_TASK_LINK,			/*私有TCP任务已经创建*/
}manage_tcp_task_event;

typedef struct _manage_tcp_info
{
	int  task_flag;		/*管理TCP 连接任务标志*/
	int  tcp_listenfd;		/*管理TCP 套接字*/	
	int  tcp_connfd;		/*管理TCP 套接字*/
}manage_tcp_info;
