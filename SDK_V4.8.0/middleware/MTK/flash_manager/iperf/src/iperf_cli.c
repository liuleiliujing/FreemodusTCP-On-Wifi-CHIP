...
static uint8_t _cli_iperf_server(uint8_t len, char *param[])
{
   ...
   /* {
        IPERF_LOGI("Iperf TCP  uart1_recv_debug: Start!");
        IPERF_LOGI("Iperf TCP uart1_recv_debug Receive Timeout = 20 (secs)");
        xTaskCreate((TaskFunction_t)uart1_recv_debug_proc_task, IPERF_TASK_NAME, IPERF_TASK_STACKSIZE / sizeof(portSTACK_TYPE), g_iperf_param, IPERF_TASK_PRIO , NULL);
        is_create_task = 1;
    }*/
