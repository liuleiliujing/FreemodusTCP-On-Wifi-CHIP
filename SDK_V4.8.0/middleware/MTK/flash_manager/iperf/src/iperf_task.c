...
#include "g113i_uart1.h"
 int  g_debug_connfd = -1;

/*****************************wangzaizhong debug END***************************************************/
...
/****************************************************************************************/
manage_tcp_info g_manage_tcp_info;


/*******************************************************************************
*iperf_menage_tcp_task_delete  
删除管理TCP处理任务
*Input:
*	任务参数
*Output:
...
*	
*/
int  iperf_menage_tcp_send_data(uint8_t *pbuf, uint16_t bufLen)
{
int nbytesend=0;
uint16_t nbytes=0;
	...
	IPERF_LOGI("iperf_menage_tcp_send_data g_manage_tcp_info.tcp_connfd =%d pbuf = %x , nbytes = %d ", g_manage_tcp_info.tcp_connfd,pbuf,nbytes);
	
	if(g_manage_tcp_info.tcp_connfd != -1)
	{
		nbytesend = send(g_manage_tcp_info.tcp_connfd, pbuf, nbytes, 0);
		if (nbytesend < nbytes)
		{
	...
	{
		IPERF_LOGI("iperf_menage_tcp_send_data fail , data len = %d ", nbytes);
		return  -1;
	}
...
*	
*/
void iperf_menage_tcp_rev_data(uint8_t *pbuf, int bufLen)
{
uint32_t dsend=0;

	/*这个发送函数，加信号量保护*/
	dsend =hal_uart_send_dma(HAL_UART_1, pbuf, bufLen);
	

}

/*******************************************************************************
*iperf_menege_tcp_run_server  
私有管理TCP处理任务
*Input:
*	任务参数
*Output:
...
*	
*/
void iperf_menege_tcp_run_server(char *parameters[])
{
    int listenfd = -1;
    int connfd = -1;
    ...
    count_t tmp_count;
    int nbytes = 0; /* the number of read */
    int nbytesend = 0; /* the number of send  ---wangzaizhong test */	
    ...
    timeout.tv_sec = 1200; // set receive timeout = 120 (sec)
    timeout.tv_usec = 0;
	...
	/*私有TCP任务删除标志*/
	g_manage_tcp_info.task_flag =0;
	g_manage_tcp_info.tcp_connfd =-1;
	g_manage_tcp_info.tcp_listenfd =-1;

	
	IPERF_LOGI(" TCP Serve task  del 1  .\n");
        vTaskDelete(NULL);
    }
    // set receive timeout
	...
	/*套接字 赋值*/
	g_manage_tcp_info.tcp_listenfd =listenfd;

	printf("Iperf bind  g_manage_tcp_info.tcp_listenfd =! %x\n\r",g_manage_tcp_info.tcp_listenfd);
        ...
        do {
		  ...
		  printf("Iperf accept   =!\n\r");
            if (ipv6_flag == 0) {
			 ...
			 printf("Iperf accept   %x=!\n\r",connfd);
			 
            if (connfd != -1) {

		/***************debug  *****************/
		g_debug_connfd =connfd;
		/*************debug END *****************/		

		/*套接字 赋值*/
		g_manage_tcp_info.tcp_connfd =connfd;
			...
			if(nbytes >0)
			{
				iperf_menage_tcp_rev_data(buffer,nbytes);
			}

#if 0			
			/***************wangzaizhong test *****************/
			IPERF_LOGI("TCP Server recv data nbytes=%d\n", nbytes);
			 for (i = 0; i < nbytes; i++)
			 {
				IPERF_LOGI("%d , ", buffer[i]);
			 }

			nbytesend = send(connfd, buffer, nbytes, 0);
			if (nbytesend < nbytes) {
				IPERF_LOGI("exception nbytes = %d ", nbytes);
			}
			/*******************wangzaizhong test  end**********/
#endif 
			/*这里的数据需要发消息到manage tcp 数据处理函数，进行数据组包*/
			
                } while (nbytes > 0);

				/*如果nbytes <= 0，接收数据异常，将关闭*/
    ...
    IPERF_LOGI(" TCP Serve task  del 2  .\n");
	/*私有TCP任务删除标志*/
	g_manage_tcp_info.task_flag =0;
	g_manage_tcp_info.tcp_connfd =-1;
	g_manage_tcp_info.tcp_listenfd =-1;
...
*	
*/
void iperf_menage_tcp_task_create(void)
{
    int i=0;
  /*先判断是否有任务，若有，将任务删除，等1000MS，再判断*/
	printf("iperf_menage_tcp_task_create: Start! %d \n\r",g_manage_tcp_info.task_flag);

 	 IPERF_LOGI(" TCP Serve task  create .\n");
	if(g_manage_tcp_info.task_flag)
	{
		if(g_manage_tcp_info.tcp_connfd != -1)
		{
			close(g_manage_tcp_info.tcp_connfd);
			g_manage_tcp_info.tcp_connfd = -1;
		}
		
		if(g_manage_tcp_info.tcp_listenfd != -1)
		{
			close(g_manage_tcp_info.tcp_listenfd);
			g_manage_tcp_info.tcp_listenfd = -1;
		}
		...
		} while (g_manage_tcp_info.task_flag ==0);  

	}

	if(g_manage_tcp_info.task_flag)
	{
		IPERF_LOGI("iperf_menage_tcp_task_create  task_flag error ! ");
	}

        printf("Iperf TCP Server: Start! \n\r");
        printf("Iperf TCP Server Receive Timeout = 1200 (secs) \n\r");
        xTaskCreate((TaskFunction_t)iperf_menege_tcp_run_server, MANAGE_TCP_TASK_NAME, MANAGE_TCP_TASK_STACKSIZE / sizeof(portSTACK_TYPE), NULL, MANAGE_TCP_TASK_PRIO , NULL);
...
*	
*/
void iperf_menage_tcp_task_delete(void)
{
  ...
  /*先判断是否有任务，若有，将任务删除，等1000MS，再判断*/
	...
	if(g_manage_tcp_info.task_flag)
	{
		IPERF_LOGI("iperf_menage_tcp_task_delete  task_flag =1  ! ");
...
/*extern QueueHandle_t g_uart1_debug_read_msgQ_id;*/


/*******************************************************************************
*uart1_recv_processing  
  从串口DMA任务中传送的消息中，得到串口数据，
  将串口数据存放到接收数据缓冲区中，
*Input:
*	无；
*Output:
...
*	OK/ERROR，表示执行成功和失败
*/
int uart1_recv_debug_processing(uart1_receive_message_struct_t  *p_msg)
{
int message_len=0;
    int nbytesend = 0;
    int nbytes = 0;	
uint16_t i=0;
	...
	/*printf("uart1_recv_debug_processing  ,message_len=%d\r\n",p_msg->message_len);

	
	for(i=0;i<message_len;i++)
	{
		printf("rev_debug[%d]=%d ,",i,p_msg->receive_data[i]);
	}*/

	nbytes =p_msg->message_len;

	if(g_debug_connfd !=-1)
	{

  		nbytesend = send(g_debug_connfd, (char *)p_msg->receive_data, nbytes, 0);
	}
...
#if 0
void uart1_recv_debug_proc_task(void *param)
{
uart1_receive_message_struct_t  uart_read_data;
   
	 printf("\r\n -------uart1_recv_debug_proc_task !------\r\n");
	 
	while (1) 
	{
	
		if (xQueueReceive(g_uart1_debug_read_msgQ_id, &uart_read_data, portMAX_DELAY))
		{
			(void)uart1_recv_debug_processing(&uart_read_data);
		}
