...
void bsp_eint_key_timer_callback(TimerHandle_t xTimer)
{
    ...
    if (i>=4) {
        log_hal_info("[eint_key]no timer found, xTimer=%d\r\n",xTimer);
        return;
    }
...
void bsp_eint_key_timer_callback(uint8_t *key_number)
{
    ...
    if (ret != HAL_GPT_STATUS_OK) {
        log_hal_info("[eint_key]longpress_time start repeat timer fail, index = %d, ret=%d\r\n",index,ret);
    }
...
void bsp_eint_key_callback(uint8_t *key_number)
{
    ...
    if (eint_key_context.key_sate[index] == BSP_EINT_KEY_RELEASE) {
        ...
        if (ret != HAL_GPT_STATUS_OK) {
            log_hal_info("[eint_key]longpress_time get timer fail, index = %d, ret=%d\r\n",index,ret);
        }
        ...
        ret = hal_gpt_sw_start_timer_ms(eint_key_context.timer_handle[index], \
                                        ...
                                        (void*)(&eint_key_context.index[index]));
        if (ret != HAL_GPT_STATUS_OK) {
            log_hal_info("[eint_key]longpress_time start timer fail, index = %d, ret=%d\r\n",index,ret);
        }
        ...
        if (ret != HAL_GPT_STATUS_OK) {
            log_hal_info("[eint_key]longpress_time stop timer fail, index = %d, ret=%d\r\n",index,ret);
        }

        ret = hal_gpt_sw_free_timer(eint_key_context.timer_handle[index]);
        if (ret != HAL_GPT_STATUS_OK) {
            log_hal_info("[eint_key]longpress_time free timer fail, index = %d, ret=%d\r\n",index,ret);
        }
    } 
